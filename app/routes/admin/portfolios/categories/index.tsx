...
export default createRoute(async (c) => {
  ...
  // SSR: Ambil hanya kategori bertipe 'portfolio'
  const { results: categories } = await db.prepare(
    "SELECT * FROM categories WHERE type = 'portfolio' ORDER BY name ASC"
  ).all();
  ...
  return c.render(
    <div className="flex min-h-screen bg-gray-50 font-sans">
      <main className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <header className="md:col-span-3 border-b border-gray-200 pb-6">
          <div className="flex items-center gap-2 mb-2 text-sm">
            ...
            <a href="/admin/portfolios" className="text-gray-400 hover:text-gray-600">Portfolios</a>
            <span className="text-gray-300">/</span>
            <span className="text-gray-900 font-bold">Kategori</span>
          </div>
          <h1 className="text-3xl font-extrabold text-gray-900">Kategori Portofolio</h1>
          <p className="text-gray-500 mt-1">Kelompokkan hasil studi kasus dan proyek yang telah selesai.</p>
        </header>
        ...
        <div className="md:col-span-1">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sticky top-6">
            ...
            <form id="form-portfolio-category" className="space-y-4">
              <input type="hidden" id="c-type" value="portfolio" />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-100">
                {categories.length === 0 ? (
                  <tr><td colSpan={3} className="p-8 text-center text-gray-400">Belum ada kategori portofolio.</td></tr>
                ) : (
      ...
      <script dangerouslySetInnerHTML={{
        __html: `
          ...
          document.getElementById('form-portfolio-category').addEventListener('submit', async (e) => {
            e.preventDefault();
          ...
          document.querySelectorAll('.btn-delete').forEach(btn => {
            btn.addEventListener('click', async (e) => {
              if(!confirm('Hapus kategori portofolio ini?')) return;
    ...
    { title: 'Kategori Portofolio - Admin' }
  )
})
