...
export default createRoute(async (c) => {
  ...
  // Mengambil data user langsung dari database (tanpa fetch internal)
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-slate-200 p-8">
        <form id="form-profile" className="space-y-8">
          ...
          {/* Email (Satu-satunya yang tetap dikunci demi keamanan auth) */}
          ...
          {/* Form Profil yang sudah BISA DIUBAH */}
          <div className="pb-8 border-b border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <input type="text" id="crm_name" defaultValue={crmData.name || ''} className="w-full border border-slate-300 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 outline-none transition" placeholder="Cth: John Doe" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Nama Perusahaan / Institusi</label>
                <input type="text" id="crm_company" defaultValue={crmData.company || ''} className="w-full border border-slate-300 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 outline-none transition" placeholder="Cth: PT Inovasi Digital" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Nomor Telepon / WhatsApp</label>
                <input type="text" id="crm_phone" defaultValue={crmData.phone || ''} className="w-full border border-slate-300 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 outline-none transition" placeholder="Cth: 08123456789" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Alamat Domisili / Penagihan</label>
                <input type="text" id="crm_address" defaultValue={crmData.address || ''} className="w-full border border-slate-300 rounded-lg p-3 focus:ring-2 focus:ring-indigo-500 outline-none transition" placeholder="Alamat lengkap..." />
              </div>
            </div>
          </div>
          ...
          document.getElementById('form-profile').addEventListener('submit', async (e) => {
            ...
            try {
              ...
              if(res.ok) {
                ...
                document.getElementById('new_password').value = ''; // Kosongkan password setelah sukses
              } else {
